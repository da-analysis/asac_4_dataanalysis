...
display(dbutils.fs.ls('dbfs:/FileStore/amazon/metadata/v2/meta_Cell_Phones_and_Accessories_v2/'))
...
meta_Cell_v2['also_buy'] = meta_Cell_v2['also_buy'].apply(lambda x: None if x.size == 0 else x)
meta_Cell_v2['also_view'] = meta_Cell_v2['also_view'].apply(lambda x: None if x.size == 0 else x)
meta_Cell_v2['description'] = meta_Cell_v2['description'].apply(lambda x: None if x.size == 0 else x)
meta_Cell_v2['feature'] = meta_Cell_v2['feature'].apply(lambda x: None if x.size == 0 else x)
meta_Cell_v2['imageURLHighRes'] = meta_Cell_v2['imageURLHighRes'].apply(lambda x: None if x.size == 0 else x)

# COMMAND ----------

meta_Cell_v2['brand'] = meta_Cell_v2['brand'].apply(lambda x: None if x=="" or x == "Unknown" else x)

# COMMAND ----------

meta_Cell_v2['similar_item'] = meta_Cell_v2['similar_item'].apply(lambda x: None if x is None or len(x) == 0 else x)
meta_Cell_v2['title'] = meta_Cell_v2['title'].apply(lambda x: None if x is None or len(x) == 0 else x)
...
# 함수 적용 예시
# meta_Cell_v2['also_buy'] = meta_Cell_v2['also_buy'].apply(make_null)
...
meta_Cell_v2['cat1'] = meta_Cell_v2['category'].apply(lambda x: x[0] if len(x) >= 1 else None)
meta_Cell_v2['cat2'] = meta_Cell_v2['category'].apply(lambda x: x[1] if len(x) >= 2 else None)
meta_Cell_v2['cat3'] = meta_Cell_v2['category'].apply(lambda x: x[2] if len(x) >= 3 else None)
meta_Cell_v2['cat4'] = meta_Cell_v2['category'].apply(lambda x: x[3] if len(x) >= 4 else None)
...
path = "dbfs:/FileStore/amazon/metadata/v2/meta_Cell_Phones_and_Accessories_v2/"

# COMMAND ----------

meta_Cell_v2_fin.to_parquet('%s/to_parquet/meta_Cell_v2.parquet' % path, compression='zstd')
...
# Parquet 파일 불러오기
path = "dbfs:/FileStore/amazon/metadata/v2/meta_Cell_Phones_and_Accessories_v2/to_parquet/meta_Cell_v2.parquet"
...
df = spark.read.parquet("dbfs:/FileStore/amazon/metadata/v2/meta_Cell_Phones_and_Accessories_v2/to_parquet/meta_Cell_v2.parquet")
...
table_name = "asac.meta_Cell_Phones_and_Accessories_v2"
